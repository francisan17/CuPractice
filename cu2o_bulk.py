...
def cu2o111(n_layers, vacuum):
  x = cu2o_bulk()
  x.translate(np.array([1.0, 1.0, 1.0])*1.0)
  x.wrap()
  slab = surface(x, (1,1,1), n_layers, vacuum=vacuum, periodic=True) 
  #bottom_O_z = np.min(slab[slab.symbols=='O'].positions[:,2])
  #maskO=slab.positions[:, 2] <= bottom_O_z 
  #del slab[maskO]
  return slab 
  
def CuD_FCC111(n_layers, vacuum):
...
def STO_FCC111(n_layers, vacuum):
    slab = cu2o111(n_layers, vacuum)
...
def CuDO_FCC111(n_layers, vacuum):
    slab = cu2o111(n_layers, vacuum)
    slab=make_supercell(slab, [[2,-1,0], [-1,2, 0],  [0,0,1]] )
    STO_z = np.max(slab[slab.symbols=='O'].positions[:,2])
    mask2 = (slab.positions[:, 2] >= STO_z) & (slab.symbols=='O')
...
def py111(n_layers, vacuum):
    slab = CuD_FCC111(n_layers, vacuum)
    slab=make_supercell(slab, [[2,-1,0], [-1,2, 0],  [0,0,1]] )
